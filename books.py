def add_book(catalogue, new):
    ...
    catalogue, or 0 otherwise. Parameter 'new' must be a dictionary with \
    keys: 'path', 'title', 'author', 'publication', 'genre', 'description', \
    'year'"""
    if new in catalogue:
        return 1
...
def search_book(catalogue, title, author):
    """Returns a dictionary corresponding to a book with 'title' written by \
    'author'. If not found - returns None"""
    for x in catalogue:
...
def edit_book(catalogue, old_title, old_author, new):
    """Replaces a book with a new one. Returns new book in case of success, \
    or None if old book is not found"""
...
def delete_book(catalogue, title, author):
    """Removes a book from a catalogue. Returns 0 in case of success, or 1 \
...
def filter_by_year(book_list, year_from, year_to, desc):
    """Filters books from book list, published between 'year_from' and \
    'year_to' and sorted according to 'desc' parameter (see filter_books \
    description).
    In next versions function will be private: usage outside this module is \
    not recommended"""
...
def filter_books(catalogue, key, value, year_from=None, year_to=None, desc=0):
    """Filters books by the key and/or publication year. Key must be "author",\
     "genre", or "year". Value is author's or genre's name; if key="year", \
     any value is allowed. If year_from=None, filtering by year is not \
     performed. If year_to=None and not year_from=None, returns only books \
     published in year_from. If desc=1, books are sorted by publish year \
     increasing; if desc=-1, are sorted by year decreasing; otherwise sorting \
     is not performed"""
